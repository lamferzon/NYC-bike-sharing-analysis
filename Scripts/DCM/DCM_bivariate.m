...
%construct X
X=ones(daily_data.num_stations,c1,d); %costant covariate
% weather data
...
%Data transform
bivariate_model.stem_data.log_transform;
bivariate_model.stem_data.standardize;
...
sqrt(mean(bivariate_model.stem_validation_result{1, 1}.cv_mse_s))
%median(bivariate_model.stem_validation_result{1, 1}.cv_R2_s)
sqrt(mean(bivariate_model.stem_validation_result{1, 2}.cv_mse_s))
%median(bivariate_model.stem_validation_result{1, 2}.cv_R2_s)

Res_pickups=bivariate_model.stem_validation_result{1, 1}.res_back;
Res_duration=bivariate_model.stem_validation_result{1, 2}.res_back;
MSE_s_p=ones(15,1);
RMSE_s_p=ones(15,1);
MSE_s_d=ones(15,1);
RMSE_s_d=ones(15,1);
...
save('.\Risultati\bivariatefull','bivariate_model')