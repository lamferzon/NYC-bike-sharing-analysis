...
%construct X
X=ones(daily_data.num_stations,c,d); %costant covariate
% weather data
...
X(1:daily_data.num_stations,4,1:d)=daily_data.distances{1}; % distances
% Lockdown and Holidays
X(1:daily_data.num_stations,5,1:d)=Holidays;
...
data.X_z{1} = [ones(daily_data.num_stations, 1) X(:,4,1) X(:,5,1)];
...
selected_trip_model = stem_model(obj_stem_data, obj_stem_par); % creo il modello con dentro sia i dati che i parametri
clear data

%Data transform
selected_trip_model.stem_data.log_transform;
selected_trip_model.stem_data.standardize;

%Starting values
obj_stem_par.beta = selected_trip_model.get_beta0();
obj_stem_par.theta_p = 0.01; %km
obj_stem_par.v_p = 1;
obj_stem_par.sigma_eta = diag([0.02 0.05 0.02]);
obj_stem_par.G = diag([0.9 0.8 0.7]);
obj_stem_par.sigma_eps = 0.3; %varianza epsilon
 
selected_trip_model.set_initial_values(obj_stem_par);
...
selected_trip_model.EM_estimate(obj_stem_EM_options);
selected_trip_model.set_varcov % calcola la matrice di varianza e covarianza di theta(perametri) quindi 
% stem mi fornisce l'incertezza sulla stima dei parametri
selected_trip_model.set_logL;
selected_trip_model.print;

selected_trip_model.stem_EM_result.stem_kalmansmoother_result.plot;
%saveas(gcf,'selected_trip_model.png')

sqrt(mean(selected_trip_model.stem_validation_result{1, 1}.cv_mse_s))
median(selected_trip_model.stem_validation_result{1, 1}.cv_R2_s)
Res_pickups=selected_trip_model.stem_validation_result{1, 1}.res_back;
MSE_s_p=ones(15,1);
RMSE_s_p=ones(15,1);
...
%save('.\Risultati\fulltrip','selected_trip_model')