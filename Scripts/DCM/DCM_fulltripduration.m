...
fulltrip_model = stem_model(obj_stem_data, obj_stem_par); % creo il modello con dentro sia i dati che i parametri
clear data

%Data transform
fulltrip_model.stem_data.log_transform;
fulltrip_model.stem_data.standardize;

%Starting values
obj_stem_par.beta = fulltrip_model.get_beta0();
...
fulltrip_model.set_initial_values(obj_stem_par);
...
fulltrip_model.EM_estimate(obj_stem_EM_options);
fulltrip_model.set_varcov % calcola la matrice di varianza e covarianza di theta(perametri) quindi 
% stem mi fornisce l'incertezza sulla stima dei parametri
fulltrip_model.set_logL;
fulltrip_model.print;

fulltrip_model.stem_EM_result.stem_kalmansmoother_result.plot;

sqrt(mean(fulltrip_model.stem_validation_result{1, 1}.cv_mse_s))
median(fulltrip_model.stem_validation_result{1, 1}.cv_R2_s)
Res_pickups=fulltrip_model.stem_validation_result{1, 1}.res_back;
MSE_s_p=ones(15,1);
RMSE_s_p=ones(15,1);
for i=1:15
    sum_res_s_p=0;
...
%save('.\Risultati\fulltrip','fulltrip_model')
